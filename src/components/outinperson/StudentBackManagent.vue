<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>学生返校登记管理</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <!-- 学生返校登记列表区域 -->
      <el-table :data="StudentBackList" border stripe>
        ...
        <el-table-column label="返校时间" prop="outTime"></el-table-column>
...
export default {
  ...
  created() {
    this.getStudentBackList()
  },
  methods: {
    async getStudentBackList() {
      ...
      const { data: res } = await this.$http.get(
        `studentBackRegistration/${this.queryInfo.pagenum}/${this.queryInfo.pagesize}`
      )
      console.log(res)
      res.data.pageData.forEach(element => {
        element.outTime = this.formatTime(element.outTime)
      })
      this.StudentBackList = res.data.pageData
      setTimeout(() => {
        loading.close()
      }, 800)
    },
